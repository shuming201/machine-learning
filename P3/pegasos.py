...
def objective_function(X, y, w, lamb):
    ...
    """
    N, D = X.shape
    obj_value = (lamb / 2) * np.dot(np.transpose(w), w) + (1 / N) * np.sum(np.max(np.hstack((np.zeros((N, 1)), 1 - np.multiply(y, np.dot(X, w)))), axis=1))
    """
    N, D = X.shape
    Y = np.maximum(0.0,1.0 - np.multiply(np.matmul(X,w).transpose(),y))
    obj_value = lamb/2 * np.sum(w*w) + float(np.sum(Y))/float(N)
...
def pegasos_train(Xtrain, ytrain, w, lamb, k, max_iterations):
    ...
    for iter in range(1, max_iterations + 1):
        ...
        # you need to fill in your solution here
        nt = 1.0 / (lamb * float(iter))
        wt = (1.0 - nt*lamb) * w
        for i in A_t:
            if (ytrain[i] * np.dot(Xtrain[i], w) < 1): #At+ set
        ...
        if wtnorm == 0:
            ...
            w = min(1.0,1.0 / np.sqrt(lamb * wtnorm)) * wt
            """
        #using np.linalg.norm(wt) and np.sum(wt*wt) result in different precision, which one is better?
        w = min(1.0,1.0 / np.sqrt(lamb * np.sum(wt*wt))) * wt    
...
def pegasos_test(Xtest, ytest, w_l):
    ...
    prediction = np.matmul(Xtest, w_l)
    n = 0
    for i in range(0, len(ytest)):
        if(prediction[i] * ytest[i] >= 0):
            n += 1
    test_acc = float(n)/float(len(ytest))
