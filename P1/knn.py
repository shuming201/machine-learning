...
def compute_distances(Xtrain, X):
    ...
    #####################################################
    #				 YOUR CODE HERE					                    #
    dists = np.sqrt(np.sum(Xtrain ** 2, axis=1) + np.sum(X ** 2, axis=1)[:, None] -2 * (X @ Xtrain.T))
...
def predict_labels(k, ytrain, dists):
    ...
    for i in range(dists.shape[0]):
        dists_sliced = np.argpartition(dists[i, :], k)
        ytrain_sliced = ytrain[dists_sliced[:k]]
        nn, number = np.unique(ytrain_sliced, return_index=False, return_counts=True)
        nn_number = dict(zip(nn, number))                     #convert into dictionary
        #nn_sorted = []
        nn_sorted = sorted(nn_number.items(), key=lambda x: ((-1) * x[1], x[0]))
        nn_label = nn_sorted[0][0]
        ypred.append(nn_label)
...
def find_best_k(K, ytrain, dists, yval):
    ...
    for k in K:
        ...
        #for each k the predicted label
