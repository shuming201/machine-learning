...
def linear_regression_noreg(X, y):
    ...
    #####################################################
    #				 YOUR CODE HERE					                    #
    w = np.asarray(np.linalg.pinv(X.T @ X) @ X.T @ y.T)
    w = w.ravel()
    #####################################################
    return w
...
def regularized_linear_regression(X, y, lambd):
    ...
    w = np.asarray(np.linalg.pinv(X.T @ X + lambd * I) @ X.T @ y.T)
    w = w.ravel()
    #####################################################
    return w
...
def tune_lambda(Xtrain, ytrain, Xval, yval, lambds):
    ...
    test_error = []
    for lambd in lambds:
        w = np.linalg.pinv(Xtrain.T @ Xtrain + lambd * I) @ Xtrain.T @ ytrain.T  
        error = np.average(np.square(np.asarray(yval.T - Xval @ w)))
        test_error.append(error)
    bestlambda = lambds[test_error.index(min(test_error))]
    #####################################################
    return bestlambda
...
def test_error(w, X, y):
    """
      ...
      """
    err = np.average(np.square(np.asarray(y.T - X @ w.T)))
    return err
